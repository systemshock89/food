...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const modalTimerId = setTimeout(() => Object(_modules_modal__WEBPACK_IMPORTED_MODULE_1__["openModal"])('.modal', modalTimerId), 50000); // показывать попап через 50 сек после открытия страницы
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', '2023-05-11');
...
function calc() {
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // пересчитывает рез-т
  ...
  calcTotal(); // получение инф-ции со статических блоков
  ...
  getDynamicInformation('#age'); // /Calc
} // module.exports = calc; // старый синтаксис экспорта модулей


/* harmony default export */ __webpack_exports__["default"] = (calc); // стандарт ES6
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
                ...
                margin: 0 auto;
            `;
      form.insertAdjacentElement('afterend', statusMessage); // соберем все данные с формы в формат FormData
      // ! у всех input-ов обязательно должен быть атрибут name

      const formData = new FormData(form); // из FormData получим обычный объект (новый способ)
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // из FormData получим обычный объект (классический способ перебора)
      // const object = {};
  ...
  function showThanksModal(message) {
    ...
    thanksModal.innerHTML = `
        ...
        `;
    document.querySelector('.modal').append(thanksModal); // спустя 4 сек убирать сообщение и возвращать форму к изначальному состоянию
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    ...
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      openModal(modalSelector, modalTimerId); // как только один раз пользователь долистал до конца и ему показался попап, обработчик удаляется, чтоб больше попап не показывался
...
function slider({
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    indicators.append(dot); // активный эл-т
  ...
  next.addEventListener('click', () => {
    ...
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      ...
      offset += deleteNotDigits(width); // offset += +width.slice(0, width.length - 2);
    }
